...
import { Send, Paperclip, MoreVertical, Phone, Video, FileIcon, Download } from "lucide-react"
...
export function ChatArea({ conversationId, currentUser }: ChatAreaProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribeTyping = ws.onTyping?.((status) => {
      if (status.conversationId === conversationId && status.userId !== currentUser.id) {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    ws.sendMessage?.(conversationId, newMessage, { id: message.id, sender: currentUser })
    setNewMessage("")
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="h-16 border-b border-border flex items-center justify-between px-6 bg-card">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-sm">{conversationId === "1" ? "Alice Johnson" : "Project Team"}</h2>
            <p className="text-xs text-muted-foreground">
              {isTyping ? "typing..." : conversationId === "1" ? "Online" : "3 members"}
            </p>
          </div>
        </div>
            ...
            return (
              <div key={message.id} className={`flex gap-3 ${isCurrentUser ? "flex-row-reverse" : ""}`}>
                ...
                <div className={`flex flex-col ${isCurrentUser ? "items-end" : "items-start"} max-w-[70%]`}>
                  {!isCurrentUser && <span className="text-xs text-muted-foreground mb-1">{message.sender.name}</span>}
                  <div
      ...
      <div className="border-t border-border p-4 bg-card">
        <form onSubmit={handleSendMessage} className="flex items-end gap-2">
          ...
          <div className="flex-1 relative">
            <Input placeholder="Type a message..." value={newMessage} onChange={handleInputChange} className="pr-4" />
          </div>
